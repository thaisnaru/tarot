...
src/engine/      deck.js (acesso/lookup dos dados), mastery.js (domínio por item+skill),
                 ...
                 shuffle.js, distractors.js, generators/ (24 arquivos, um por tipo de pergunta, + index.js)
...
| 4 | Naipes (4 naipes, sem emoji no prompt — vazava o elemento) | suit | naipe |
...
## Tipos de pergunta (24 geradores em `src/engine/generators/`)
...
**Contextualizados/anti-spoiler (10):** reconhecimentoCarta, cartaConceito, reconhecimentoSimbolo, simboloCarta, corCarta, numeroCartaMaior, naipeCarta, uprightReversed, significadoCarta, detetiveSimbolos.

**Naipe, rodada 2026-07-26 (3):** conceitoNaipe (esfera de vida → naipe), naipeAplicacao (cenário derivado das keywords do naipe → naipe), naipeComparacao (compara 2 naipes na mesma pergunta, todas as alternativas citam os dois nomes). Motivo: os geradores de naipe originais vazavam a resposta (`naipe-significado` usava texto que começava com o nome do próprio naipe; o prompt mostrava emoji do elemento).
...
- `symbolsForPool`/`appears_in` em `symbols.json` só referencia Arcanos Maiores (Menores não têm símbolo individual catalogado — ver ressalva do dataset semente). Qualquer gerador novo que dependa de símbolo de uma carta específica precisa checar isso antes de gerar pra carta menor, senão quebra (foi o caso do bug corrigido em 2026-07-26 no `reconhecimento-simbolo` — ver Log de decisões do SPEC.md).
